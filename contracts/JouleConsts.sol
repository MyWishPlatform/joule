...
contract usingConsts {
    ...
    uint constant REMAINING_GAS = 20000;
    ...
    // there is correlation between amount of registered gas and required gas to invoke - 3%
    uint constant JOULE_INVOKE_GAS_PERCENT = 3;
    ...
    // Code version
    bytes8 constant VERSION = 0x010800ad086575bc;
    //                          ^^ - major
    //                            ^^ - minor
    //                              ^^^^ - build
    //                                  ^^^^^^^^ - git hash
